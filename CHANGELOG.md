...
- **Context menus that are actually reachable, including on a phone.** Right-click worked on table rows and on a 40-pixel strip below the listing, and nowhere else — not on the empty space that makes up most of a sparse directory, and the phone layout had no context menu at all. The whole listing area is a target now, with padding below the content so the background menu stays reachable in a directory of any length; the phone's cards and the grid's tiles have the same menu, opened by holding one. Holding tolerates about ten pixels of drift, since a finger resting on glass is never quite still.
