...
전체 화면 터미널에 손가락 스크롤 입력을 전달합니다. **AI 코딩의 tmux 기록 스크롤에는 서버 v0.73.1 이상도 필요합니다.** v0.73.0 서버는 tmux 마우스 입력을 꺼두므로 앱 업데이트만으로 AI 기록 스크롤을 해결하지 못합니다. 서버 업데이트 후 기존 세션에 다시 접속하면 적용되며 세션을 삭제할 필요는 없습니다.
