...
- **특수 키:** 한 줄에 작성·Shift·Ctrl·Esc·Tab·Enter·도구를 표시합니다. `도구`를 누르면 터미널을 가리지 않는 하단 조작 패널이 펼쳐집니다. 이동·조합키·출력 탭에서 방향키·Alt·페이지 이동·Ctrl 조합·읽기·검색을 사용합니다. `접기` 또는 뒤로 가기로 닫습니다. Shift·Ctrl·Alt를 조합해 화면 키에 적용합니다. Shift+Tab, Shift+Enter, 방향키, Page Up/Down, Home/End, Ctrl+C를 지원합니다. 조합 상태는 다음 화면 키를 누르면 해제됩니다. 휴대폰 키보드의 문자 대소문자는 키보드 자체 Shift로 선택합니다.
