...
- **출력 탐색:** `키` 메뉴의 `기록 위로 / 아래로`는 xterm 출력 기록을 이동하며 프로그램에 키를 보내지 않습니다. `PgUp / PgDn`은 실행 중인 프로그램으로 전달됩니다. `최신 출력`으로 돌아갈 수 있습니다. 전체 화면 프로그램의 자체 기록은 해당 프로그램의 PgUp/방향키를 사용하세요.
