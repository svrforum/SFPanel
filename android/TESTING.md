...
## 화면 정리 — 키 바·서버 목록·전체 기능 (2026-09-19)

- API 35 에뮬레이터와 임시 health/WebSocket fixture 서버로 확인했습니다. 화면 요소는 `uiautomator dump`의 레이블·설명·좌표로, 전송 바이트는 로그로 확인했고 색·모양은 첨부한 스크린샷으로만 남깁니다.
- 키 바는 두 줄입니다. 첫 줄 `←`(`ESC[D`)·`↑`·`↓`·`→`·`Esc`·`Tab`·`Enter`, 둘째 줄 `Shift`·`Ctrl`·`Alt`·`작성`·`도구`를 덤프에서 확인했습니다. 방향키는 도구를 펼치지 않고 바로 누를 수 있습니다.
- `Shift` → `←` 순서로 눌러 `ESC[1;2D` 전송을 확인했고, 조합 없이 누른 `←`는 `ESC[D`였습니다.
- 전체 기능은 15개 타일입니다. `작업 공간`에 대시보드·터미널·파일 관리, `앱과 서비스`에 6개, `서버와 시스템`에 6개가 들어가며 `AI 코딩` 항목은 없습니다. 이름과 경로가 어긋나지 않는지는 `터미널` 타일을 눌러 상단 제목이 `design · 터미널`로 바뀌는 것으로 확인했습니다.
- 마지막 줄이 가려지지 않습니다. 목록을 끝까지 내리면 `클러스터 · 노드`와 `서버 설정`이 `[89,1821][535,1958]`, `[545,1821][991,1958]`로 온전히 보이고, 그 아래 `내 서버`·`보기 설정` 줄은 `y=1963`에서 시작해 겹치지 않습니다.
- `/terminal`의 `⋯`는 9개 항목입니다. v0.76.0에서 AI 화면이 터미널로 합쳐졌으므로 `AI 도구·계정 설정 열기/닫기`와 `세션 관리`도 이 화면의 메뉴입니다. 번호가 밀리지 않았는지 `터미널 조작`으로 도구가 펼쳐지는 것과 `키보드 열기`로 WebView 높이가 1802 → 980px로 줄어드는 것을 확인했습니다.
- `docs/`의 서버 목록·전체 기능·키보드·접이식 도구 스크린샷을 같은 에뮬레이터에서 다시 찍어 교체했습니다.
- 실제 휴대폰, TalkBack, 실제 tmux 세션의 AI 도구 패널은 이번 검증에 포함하지 않았습니다.

## 큰 글꼴 — 시스템 글자 크기 200% (2026-09-19)

- `settings put system font_scale 2.0`을 적용한 같은 에뮬레이터에서 확인했습니다.
- 키 바 두 줄의 캡이 모두 `h=116`(44dp)이고 첫 줄은 `y=2074`, 둘째 줄은 `y=2200`에서 시작합니다. 줄이 어긋나거나 글자가 잘려 나가지 않습니다. 캡 너비에 들어가지 않는 `Enter`는 캡에 맞춰 글자 크기를 줄여 전체 레이블을 보여 줍니다.
- 이 상태에서 `Shift` → `←` 순서로 눌러 `ESC[1;2D` 전송을 다시 확인했습니다.
- `서버 연결하기` 버튼은 126 → 149px로 늘어나 글자를 자르지 않습니다. 글자 크기 100%에서는 126px 그대로입니다.
