...
설치 후:
1. `http://<서버IP>:19443` 접속
2. 셋업 위저드에서 관리자 계정 생성
3. **설정 → 2단계 인증 → 2FA 활성화** (권장)
4. 프로덕션이라면 reverse proxy + TLS로 패널 앞단을 감싸고, 방화벽으로 19443을 LAN/VPN으로만 제한
...
# 3. 다른 노드에서 클러스터 참여
sudo sfpanel cluster join 10.0.0.1:9444 <token>
```
...
[
  {"id":"<node-uuid-1>","address":"<ip-1>:9445","non_voter":false},
  {"id":"<node-uuid-2>","address":"<ip-2>:9445","non_voter":false}
]
EOF
...
```bash
# 터미널 1: 프론트엔드 (핫 리로드, API 프록시 → :19443)
cd web && npm run dev
...
- **역방향 프록시 + TLS** — 프로덕션은 Nginx/Caddy/Cloudflare Tunnel 등으로 HTTPS를 적용. 번들된 19443 포트는 plain HTTP입니다.
- **접근 제한** — 방화벽(UFW)으로 19443/9444/9445를 신뢰 IP/CIDR에만 허용
