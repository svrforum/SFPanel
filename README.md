...
> **검증 환경** — **Ubuntu 24.04 LTS / 26.04 LTS**에서 테스트했습니다. 다른 Debian/Ubuntu 계열에서도 대부분 동작하지만(systemd + apt 기반), 공식 검증은 위 두 버전 기준입니다. 그 외 배포판(RHEL/Fedora, Arch 등)은 미검증이며 패키지 관리·서비스 제어 일부 기능이 다를 수 있습니다.
...
- SHA-256 체크섬 검증 후 바이너리를 `/usr/local/bin/sfpanel`에 설치
- `/etc/sfpanel/config.yaml` 생성 (32바이트 랜덤 JWT 시크릿, 모드 0600)
- systemd 유닛 등록 (`Restart=always`, `MemoryHigh=1G`, `TasksMax=4096`, `PrivateTmp=true`)
- logrotate 설정 (`/etc/logrotate.d/sfpanel`, 일별 7개 보관)
...
systemd가 없거나 설치 스크립트를 쓰고 싶지 않을 때:
...
# 3. 최소 config.yaml 작성 (JWT 시크릿은 반드시 고유한 값으로)
...
> 수동 설치 환경에서는 클러스터 탈퇴/해산, 패널 업데이트 같은 핸들러가 의도적으로 종료되어 supervisor가 재기동해야 합니다. 가능하면 systemd 유닛을 함께 작성하거나 설치 스크립트로 옮기세요.
...
cluster:
  ...
  node_id: ""
  node_name: ""
  grpc_port: 3629                # Raft 전송은 자동으로 grpc_port + 1 (3630)
...
| 변수 | 설명 | 기본값 |
|------|------|--------|
| `SFPANEL_PORT` | 서버 포트 | 3628 |
| `SFPANEL_JWT_SECRET` | JWT 서명 시크릿 | config.yaml |
| `SFPANEL_DB_PATH` | SQLite DB 경로 | config.yaml |
| `SFPANEL_LOG_LEVEL` | 로그 레벨 (`debug`/`info`/`warn`/`error`) | `info` |
| `GOMAXPROCS` | Go 동시 실행 코어 수 | 2 (홈서버 기본) |
| `GOGC` | Go GC 트리거 비율(%) | 50 (메모리 절약) |

> 8코어+ 클러스터 호스트에서 클러스터 fanout이 많다면 `GOMAXPROCS=4` 또는 `GOMAXPROCS=8` 정도로 풀어주는 것이 좋습니다. systemd 유닛의 `[Service]` 섹션에 `Environment="GOMAXPROCS=4"` 추가 후 `systemctl daemon-reload && systemctl restart sfpanel`.
...
업그레이드 직전(설치 스크립트는 서비스 정지 후, CLI는 바이너리 교체 직전)에 `sfpanel.db.bak-<YYYYmmdd-HHMMSS>` 형태로 스냅샷을 자동 생성합니다. 최근 3개를 보관하고 그 이전은 자동 삭제됩니다. 마이그레이션 회귀로 패널이 부팅 안 될 때 롤백:
...
여러 노드를 동시에 재시작하면 mTLS 핸드셰이크 + Raft pre-vote 흐름 때문에 **15~20초간 leader 재선출 지연**이 발생할 수 있습니다. 무중단 업그레이드는 노드 한 대씩 처리하고 사이에 10초 이상 간격을 두세요. 또는 웹 UI의 **클러스터 → 업데이트 → 롤링 모드**(SSE 진행률 스트리밍)를 사용하면 자동으로 한 노드씩 처리합니다.
...
빌드·설치 방법과 검증 범위는 [Android 앱 안내](android/README.md)를 참고하세요. [GitHub 릴리즈](https://github.com/svrforum/SFPanel/releases/tag/android-v0.1.2)에서 서명된 APK를 받을 수 있습니다. `android-v*` 태그를 push하면 테스트와 서명 검증 후 자동 배포됩니다.
...
데스크탑 앱은 **서버와 동일 버전 라인**으로 출시됩니다 (v0.13.5+, 그 이전 데스크탑은 `0.6.x` 별도 라인). 새 버전이 release 페이지에 올라오면 앱이 자동으로 감지하고 업데이트 확인 다이얼로그를 띄웁니다 (ed25519 서명 검증, `tauri-plugin-updater`). 수동 재설치 없이 그 자리에서 업데이트하면 됩니다.
...
| 항목 | 수명 | 회전 방법 |
|------|------|-----------|
| 클러스터 CA | 10년 | 코디네이션된 재시작 (전 노드 동시 신뢰 갱신) |
| 노드 cert | 5년 | `sudo sfpanel cluster reissue-cert` (재시작 불필요, ≤1분 내 반영) |

### 쿼럼 손실 복구 (peers.json)

하드웨어 장애나 노드 영구 분실로 잔존 voter 수가 쿼럼에 못 미쳐 정상 멤버십 변경이 불가능할 때:

```bash
# 잔존 노드의 /var/lib/sfpanel/cluster/peers.json 작성:
...
바이너리와 서비스만 제거합니다. 설정(`/etc/sfpanel`)과 데이터(`/var/lib/sfpanel`)는 보존되니, 완전 제거하려면 다음을 추가로 실행:
...
```bash
# 터미널 1: 프론트엔드 (핫 리로드, API 프록시 → :3628)
cd web && npm run dev
...
- **로그인 보호** — 5회 실패 시 5분간 IP 차단 (내장 rate limiting)
- **JWT 시크릿** — 32바이트 랜덤 자동 생성 + config.yaml 0600 권한
- **클러스터 mTLS** — 노드 간 gRPC/Raft 통신 자동 암호화 (CA 자동 발급)
- **조인 토큰** — 시간 제한 HMAC 서명, 일회성
- **systemd 하드닝** — `MemoryHigh=1G`, `TasksMax=4096`, `PrivateTmp=true`, `RestrictSUIDSGID=true` (`scripts/install.sh`로 설치된 유닛 기준)
- **민감 디렉토리 권한** — `/etc/sfpanel/` 0700, `config.yaml` 0600, `/etc/sfpanel/cluster/*.key` 0600, `/var/lib/sfpanel/` 0700

## 후원 / 링크

SFPanel이 유용하다면 커피 한 잔으로 개발을 응원해주세요. 패널 사이드바 하단에서도 동일한 후원 / 깃허브 링크를 확인할 수 있습니다.
...
[GNU AGPL-3.0](LICENSE) — 셀프호스팅·수정·재배포 모두 자유지만, 수정한 버전을 **네트워크 서비스로 제공**할 경우 그 소스 코드도 공개해야 합니다(§13). 정직한 셀프호스터에게는 추가 의무가 없습니다.

> **상용/클로즈드 라이선스 문의** — AGPL 조건 없이 임베드·재판매하고 싶다면 별도 상용 라이선스가 가능합니다. <svrforum.com@gmail.com> 으로 문의하세요.
