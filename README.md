...
- **서비스 관리** — systemd 서비스 시작/중지/재시작/활성화
...
- **셀프 관리** — 웹 업데이트 (SSE 스트리밍), 설정 백업/복원 (DB + config + Compose 파일)
- **보안** — JWT 인증 + TOTP 2FA, bcrypt 해시

## 아키텍처

```
Go Binary (Echo v4)
├── REST API (154+ endpoints) + WebSocket (5 endpoints)
├── Embedded React SPA (go:embed)
├── SQLite (설정, 인증, 커스텀 로그소스)
...
| UI | Recharts (차트), xterm.js (터미널), Monaco Editor (코드 에디터) |
...
백업에 포함되는 항목:
- `sfpanel.db` — 관리자 계정, 세션, 설정, Compose 프로젝트 메타데이터
...
인증: `Authorization: Bearer <JWT>` 헤더. WebSocket/다운로드는 `?token=<JWT>` 쿼리 파라미터.
