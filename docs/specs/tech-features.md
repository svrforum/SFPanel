...
- **주요 기능**:
  ...
  - **계정별 실행**: 허용 목록은 패널 계정 + uid ≥ 1000 로그인 계정. 다른 계정은 `runuser -u <계정> --`로 실행하고 CLI 설치 상태·버전도 그 계정의 대화형 로그인 셸(`bash -lic`) 기준으로 조회 — `~/.local/bin`은 `.bashrc`에서 PATH에 붙고, 데비안 기본 `.bashrc`는 비대화형 셸에서 그 줄 앞에서 빠져나간다
  - **프로파일(도구 설정 디렉터리)**: 같은 CLI로 계정을 여러 개(개인/업무 OpenAI, 고객사별 Anthropic) 쓰기 위해 세션마다 설정 디렉터리를 고른다 — Codex `CODEX_HOME`, Claude Code `CLAUDE_CONFIG_DIR`. 각 CLI는 설정 디렉터리 한 곳에 자격증명 한 벌만 두므로 이것이 유일한 방법이다. 프로파일은 `<계정 홈>/.sfpanel-ai/<도구>/<이름>`(0700, 실행 계정 소유)이고 클라이언트는 이름만 보낸다(`/tmp` 아래가 아닌 이유: Codex가 임시 디렉터리에 헬퍼 바이너리 만들기를 거부한다). 기본 프로파일 = 도구 자신의 디렉터리(`~/.codex`·`~/.claude`)이며 환경변수를 **아무것도** 설정하지 않아 v0.73.0과 동작이 같다. 변수는 세션을 만드는 `new-session -e` 한 쌍으로만 실린다 — tmux는 클라이언트 환경을 세션에 복사하지 않으므로 프로세스 환경(`env -i …`·`systemd-run --setenv=`)에 실으면 그 값이 tmux 서버의 전역 환경이 되어 이후 '기본'으로 만든 세션까지 조용히 같은 프로파일로 돌아간다. 생성은 빈 디렉터리 하나를 만드는 것뿐 — 로그인은 세션 안에서 CLI 자신의 흐름으로 하고, 패널은 자격증명 파일을 열거나 읽거나 복사하지 않는다. 삭제는 살아있는 세션이 그 프로파일을 쓰고 있으면 `AI_PROFILE_IN_USE`(409)로 거절하고, 경로의 어느 단계도 심링크를 따라가지 않는다. Gemini는 설정 디렉터리 override를 검증한 바 없어 제외 (세션의 프로파일 열 = 마이그레이션 37)
