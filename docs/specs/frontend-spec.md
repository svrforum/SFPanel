...
- **`lib/sessionRail.ts`**: `buildRail`·`pickActive`·`findItem`·`railNote`·`activeKey`·`parseActiveKey`·`prunePtyTabs`(vitest). **`lib/aiSessions.ts`**: 그대로(바로 아래 헬퍼 항목 참고).
