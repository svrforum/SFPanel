...
- **기능**: Docker Compose 스택 관리 (목록 + 상세를 하나의 컴포넌트에서 처리)
  ...
  - **호스트 권한 확인 후 저장**: 저장/생성/임포트가 `COMPOSE_RISKY`로 거부되면(docker.sock, privileged, 호스트 네임스페이스 등) 서버가 알려준 항목을 한 번에 나열하는 확인 대화상자를 열고, 계속하면 같은 요청을 `acknowledge_risks: true`로 다시 보낸다 (`web/src/lib/composeRisk.ts` — 네 개의 제출 경로 공통). `COMPOSE_FORBIDDEN`(패널 자신의 비밀 경로)은 대화상자 없이 에러로만 표시한다 — 어떤 답을 해도 서버가 받지 않는다
