...
- **기능**: Docker Compose 스택 관리 (목록 + 상세를 하나의 컴포넌트에서 처리)
  ...
  - **호스트 권한 확인 후 저장**: 저장/생성/임포트가 `COMPOSE_RISKY`로 거부되면(docker.sock, privileged, 호스트 네임스페이스 등) 서버가 알려준 항목을 한 번에 나열하는 확인 대화상자를 열고, 계속하면 같은 요청을 `acknowledge_risks: true`로 다시 보낸다 (`web/src/lib/composeRisk.ts` — 네 개의 제출 경로 공통). `COMPOSE_FORBIDDEN`(패널 자신의 비밀 경로)은 대화상자 없이 에러로만 표시한다 — 어떤 답을 해도 서버가 받지 않는다. 앱스토어 고급 설치도 같은 대화상자를 쓴다: 거부는 SSE 이벤트로 오므로 이벤트의 `code` 필드로 구분하고, 계속하면 `acknowledge_risks: true`로 설치를 다시 스트리밍한다 (`web/src/pages/AppStoreDetail.tsx`; 비밀번호 재확인은 그대로 별도 관문)
...
- **레이아웃**: 왼쪽 레일(272px, 접으면 56px, `sfpanel_terminal_rail`) + 큰 패널. 레일은 `role="tablist" aria-orientation="vertical"`이고 행은 `role="tab"`(활성 행 `aria-selected=true`, 화살표·Home/End 이동). 세션은 **디렉터리(정확한 cwd)별로 묶고** 그룹은 안의 가장 오래된 세션 순(새 디렉터리는 아래에 추가), 그룹 안은 서버 순서에 종료 세션을 뒤로(`lib/sessionRail.ts` `buildRail`). 임시(PTY) 그룹은 항상 마지막이며 탭이 있거나 폴백 모드일 때만 보인다 — 헤더에 수명("이 브라우저에서만 · 새로고침 후 5분")을 적고, 다시 연결할 수 있는 서버 PTY 세션(`GET /terminal/sessions`)을 그 아래 나열한다. 이 목록은 **두 모드 모두 마운트 때 한 번** 조회하고 이후 탭 수가 바뀔 때마다 다시 조회한다(이름 변경은 조회하지 않는다). tmux 모드에서도 조회하는 이유는 id 생성기다: `term-N`은 페이지 로드마다 0에서 다시 시작하는 모듈 상태인데 서버 세션은 그보다 오래(마지막 리더 이후 5분) 살아 있어서, 조회가 없으면 새 임시 셸이 `term-1`을 요구하고 서버는 이미 아는 id를 그 살아있는 셸로 돌려준다 — 이전 페이지 로드나 같은 계정의 다른 기기가 남긴 셸로 들어가게 된다. `usePtyTabs`의 `note`가 탭으로 만들지 않고 생성기만 그 id들 너머로 밀어 둔다. 행: 상태 점(`stateDotClass`, 활성 행은 펄스 없음, `motion-reduce:animate-none`) · 도구 글리프(`TOOL_META`) · 제목 · 프로파일 칩 · process 모드/우회 플래그/알 수 없음 마커 · 둘째 줄은 말할 것이 있을 때만(`railNote`: 입력 대기·도구 종료됨·종료됨 — 프롬프트에 있는 셸 세션은 정상이라 비운다). 활성 행은 도구색 2px 바. 우클릭·길게누르기 메뉴: 이름 변경·정보·다시 실행·다시 시작·목록에서 지우기·종료(PTY 행은 이름 변경·닫기).
- **활성 항목**: `sfpanel_terminal_active:<node>`에 `tmux:<id>` / `pty:<id>`로 저장(접두사 없는 옛 값은 **무시한다** — 옛 PTY 페이지는 방문자마다 첫 탭을 자동으로 만들었으므로 그 id는 운영자가 고른 세션이 아니고, 그대로 따르면 살아있는 tmux 세션을 두고 유령 임시 셸이 열렸다) → 없으면 `last_attached_at`이 최신인 살아있는 tmux 세션 → 첫 항목(`pickActive`). 서버 목록이 도착한 뒤에만 재정렬한다. 복원하는 값은 **`tmux:` 키뿐이다** — 활성 항목을 저장하는 효과가 `pty:` 키도 쓰지만, 임시 탭은 저장하지 않으므로 다음 로드에서 그 값은 더 이상 존재하지 않는 탭을 가리킨다. 페이지를 열 때 `tmux:`가 아닌 저장 값과 옛 `sfpanel_terminal_tabs:<node>` 목록은 **지운다**(옛 페이지에서 올라온 브라우저에 죽은 포인터를 남기지 않는다). 지우기가 유지되는 이유가 `parseActiveKey`가 그 값을 거부하는 것이다 — state로 들였다면 저장 효과가 곧바로 다시 썼을 것이다. 임시 탭은 **서버에 그 세션이 있거나 운영자가 이 페이지에서 방금 열었기 때문에만** 존재한다.
...
- **폴백 모드**(`tools.tmux.supported === false`): 패널 위 배너(브라우저에만 남는다는 문장 + tmux 설치 버튼), 레일에는 임시 그룹만, "새 임시 세션"이 PTY 탭(`usePtyTabs`, `term-N` — 저장하지 않는다)을 연다. 이 모드에서는 탭 목록을 `GET /terminal/sessions`에서 **유도한다**: 처음 성공한 목록을 탭으로 받아들여(`usePtyTabs`의 `adopt`) 새로고침 후에도 운영자의 셸이 그대로 돌아온다. 처음 한 번만이다 — 탭을 닫아도 서버 PTY는 5분간 살아있어서 이후 목록에 계속 나오는데, 다시 받아들이면 방금 닫은 탭이 되살아나 '다시 연결' 대신 탭으로 돌아온다. 목록 조회가 실패하면 탭을 그대로 둔다(만들지도, 지우지도 않는다). 설치가 끝나면 재조회로 tmux 모드로 돌아온다.
...
- **실행 옵션 기억**: (노드, 도구, 디렉터리)별 `localStorage`(`sfpanel_ai_launch:<node>:<tool>:<cwd>` — `launchKey`)라서 같은 폴더를 같은 도구로 다시 열면 지난번처럼 시작한다. 아무것도 고르지 않았으면 키를 지운다 — '옵션 없이'도 기억해야 한다. 도구나 계정이 바뀌면 다시 읽고 고른 값을 버리며(Claude의 `acceptEdits`는 Codex의 승인 정책이 아니고 root Claude의 우회는 서버가 거절한다), 디렉터리 변경은 섹션을 건드리지 않은 동안만 다시 읽는다(`untouchedPrefill` — 타이핑 중에 고른 옵션을 지우지 않는다). 읽은 값이 깨졌거나 프라이빗 모드에서 throw하면 '없음'으로 — 옵션이 대화상자를 열지 못하게 만들 수는 없고 서버가 다시 검증한다. 하나도 고르지 않은 요청은 `launch`를 **아예 보내지 않는다**.
...
| `sfpanel_terminal_active:<node>` | (v0.53.0) 터미널 활성 항목 `tmux:<id>` / `pty:<id>` — 노드별 네임스페이스(`<node>` = nodeId 또는 `local`). 옛 `sfpanel_terminal_tabs:<node>`(임시 탭 목록)는 더 이상 쓰지 않고 페이지를 열 때 지운다 | `Terminal.tsx` |
