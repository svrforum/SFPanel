...
- **활성 항목**: `sfpanel_terminal_active:<node>`에 `tmux:<id>` / `pty:<id>`로 저장(접두사 없는 옛 값은 **무시한다** — 옛 PTY 페이지는 방문자마다 첫 탭을 자동으로 만들었으므로 그 id는 운영자가 고른 세션이 아니고, 그대로 따르면 살아있는 tmux 세션을 두고 유령 임시 셸이 열렸다) → 없으면 `last_attached_at`이 최신인 살아있는 tmux 세션 → 첫 항목(`pickActive`). 서버 목록이 도착한 뒤에만 재정렬한다. 저장소에서 복원한 PTY 탭은 `GET /terminal/sessions`와 맞춰(`prunePtyTabs`) 서버에 없는 세션이면 목록에서 지운다 — 서버는 모르는 id로 접속하면 세션을 **새로 만들기** 때문에 죽은 탭은 '없어졌다'고 알리는 대신 셸을 하나 만들어낸다. 그래서 복원된 탭은 그 응답이 오기 전까지 패널에 마운트하지 않고(`mountablePtyTabs` — `usePtyTabs`의 `mountable`이 이 규칙이다), 요청이 실패하면 탭을 그대로 두고 보류만 푼다.
...
- **`lib/sessionRail.ts`**: `buildRail`·`pickActive`·`findItem`·`railNote`·`activeKey`·`parseActiveKey`·`prunePtyTabs`·`mountablePtyTabs`(vitest). **`lib/aiSessions.ts`**: 그대로(바로 아래 헬퍼 항목 참고).
