...
- **`lib/aiSessions.ts` 헬퍼**: `supportsProfiles`(프로파일을 지원하는 도구) · `loginCommandFor`(Codex `codex login` / Claude `/login`) · `relativeSince`(`Intl.RelativeTimeFormat`, 값이 없으면 빈 문자열) · `defaultTitle(tool, cwd, profile?)` · `aiErrorMessage`(`AI_PROFILE_EXISTS`·`AI_PROFILE_IN_USE`를 i18n 키로 매핑, 나머지 코드는 서버 메시지를 그대로 — 이름 형식 거절(`INVALID_BODY`)이 여기 해당한다)
