...
Run: `cd /opt/stacks/SFPanel && go test ./internal/cluster/ -count=1 && /home/devuser/go/bin/golangci-lint run ./internal/cluster/...`
Expected: clean.
...
Run: `cd /opt/stacks/SFPanel && go build ./... && /home/devuser/go/bin/golangci-lint run ./internal/cluster/...`
Expected: clean.
...
Run: `/home/devuser/go/bin/golangci-lint run ./internal/feature/appstore/...`
Expected: clean.
...
Run: `/home/devuser/go/bin/golangci-lint run ./internal/feature/appstore/... ./internal/api/...`
Expected: clean.
...
If you have access to the second cluster node (`192.168.1.x`):
```bash
RTOK=$(curl -sk --max-time 3 -X POST -H 'Content-Type: application/json' -d '{"username":"admin-sv","password":"<PWD>"}' http://192.168.1.x:8444/api/v1/auth/login | python3 -c "import sys,json; print(json.load(sys.stdin)['data']['token'])")
curl -s -H "Authorization: Bearer $RTOK" "http://192.168.1.x:8444/api/v1/appstore/forks" | python3 -m json.tool
```
Expected: same fork visible on the other node (FSM replication confirmed).

- [ ] **Step 5: UI smoke (Playwright or browser)**

Navigate to `http://192.168.1.x:9443/appstore`:
